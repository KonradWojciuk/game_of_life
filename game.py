...
# Start simulation
def start_simulation(tick_interval):
...
def handle_mouse_click(event):
    global game_state
    if button_x <= event.pos[0] <= button_x + button_width and button_height <= event.pos[1] <= button_y + button_height:
        next_generation()
    else:
...
# Start the simulation with a tick interval of <n> seconds
start_simulation(simulation_speed)